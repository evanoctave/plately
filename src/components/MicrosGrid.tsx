// =============================================================================
// MicrosGrid — Eight tracked micronutrients as a grid
// =============================================================================
// Renders sodium, potassium, calcium, iron, magnesium, vitamin A/C/D as a 4×2
// grid of mini cards. Each cell shows the absolute amount + a "% of FDA Daily
// Value" bar (clamped 0..1) using the metadata in `MICRO_META`. Color
// coding: under = muted, met = accent, over = amber.

import { useMemo } from 'react';
import { View, Text, StyleSheet } from 'react-native';

import { MICRO_META, type Nutrition } from '../data/nutrients';
import { radius, spacing, font, macroColors } from '../theme';
import { useTheme } from '../theme/ThemeContext';
import { clamp01 } from '../utils/nutrition';
import { fmt } from '../utils/format';

/** Renders the eight tracked micronutrients with % of FDA Daily Value. */
export function MicrosGrid({ nutrition }: { nutrition: Nutrition }) {
  const p = useTheme();
  const styles = useMemo(() => makeStyles(p), [p]);
  return (
    <View style={styles.grid}>
      {MICRO_META.map((meta) => {
        const amount = nutrition[meta.key];
        const pct = clamp01(amount / meta.dailyValue);
        const pctLabel = Math.round((amount / meta.dailyValue) * 100);
        return (
          <View key={meta.key} style={styles.cell}>
            <Text style={styles.label}>{meta.label}</Text>
            <Text style={styles.amount}>
              {fmt(amount)} {meta.unit}
            </Text>
            <View style={styles.track}>
              <View style={[styles.fill, { width: `${pct * 100}%` }]} />
            </View>
            <Text style={styles.pct}>{pctLabel}% DV</Text>
          </View>
        );
      })}
    </View>
  );
}

function makeStyles(p: ReturnType<typeof useTheme>) {
  return StyleSheet.create({
    grid: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.md },
    cell: {
      width: '47%',
      flexGrow: 1,
      backgroundColor: p.surfaceAlt,
      borderRadius: radius.md,
      padding: spacing.md,
    },
    label: { color: p.textMuted, fontSize: font.size.sm },
    amount: { color: p.text, fontSize: font.size.lg, fontFamily: font.family.monoSemibold, marginTop: 2 },
    track: {
      height: 6,
      borderRadius: radius.pill,
      backgroundColor: p.border,
      overflow: 'hidden',
      marginTop: spacing.sm,
    },
    fill: { height: '100%', borderRadius: radius.pill, backgroundColor: macroColors.fat },
    pct: { color: p.textFaint, fontSize: font.size.xs, marginTop: 4 },
  });
}
