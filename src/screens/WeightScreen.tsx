...
function makeStyles(p: ReturnType<typeof useTheme>) {
  return StyleSheet.create({
    ...
    rangeBtn: { paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: radius.pill, backgroundColor: p.surfaceAlt },
    rangeText: { color: p.textMuted, fontSize: font.size.sm, fontFamily: font.family.uiSemibold },
    rangeTextActive: { color: p.black },
    statRow: { flexDirection: 'row', gap: spacing.sm },
    stat: { flex: 1, alignItems: 'center', gap: 2 },
    statValue: { fontSize: font.size.xl, fontFamily: font.family.monoBold },
    statLabel: { color: p.textMuted, fontSize: font.size.xs },
...
export function WeightScreen() {
  ...
  const delta = current != null && first != null ? round(current - first) : null;
...
function Stat({ value, label, accent, styles }: { value: string; label: string; accent: string; styles: ReturnType<typeof makeStyles> }) {
  return (
